...
;(async () => {
  if (argv.dev || argv.d) {
    // CLI: ./node_modules/.bin/concurrently "./node_modules/.bin/ts-node-dev -O \"{\\\"module\\\":\\\"commonjs\\\"}\" ./lib/appxgen/server/react.ts"
    const startServerCommandUnix = isImported
      ? '"node ./node_modules/appxgen/build/custom/react.js"'
      : 'ts-node-dev -O "{\\"module\\":\\"commonjs\\"}" ../../lib/appxgen/server/react.ts'

    // CLI: .\node_modules\.bin\concurrently ".\node_modules\.bin\ts-node-dev -O ""{""""module"""": """"commonjs""""}"" ./lib/appxgen/server/react.ts"
